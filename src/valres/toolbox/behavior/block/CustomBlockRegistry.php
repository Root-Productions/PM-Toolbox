...
final class CustomBlockRegistry {
    ...
    private function registerBlockItem(BlockBuilder $builder): void {
        ...
        $itemBuilder = $block instanceof Crops
            ? $this->createDefaultBlockItemBuilder($builder)
            : ($item instanceof ItemBlock ? $this->createDefaultBlockItemBuilder($builder) : $builder->getItemBuilder());

        $this->registerCustomBlockItem(
            $itemBuilder,
            $builder
        );
    ...
    private function registerCustomBlockItem(ItemBuilder $builder, ?BlockBuilder $blockBuilder = null): void {
        ...
        if ($builder instanceof DataDrivenItemBuilder) {
            ...
            if ($item instanceof AutoGeneratedBlockItem) {
                $builder->removeProperty(BlockProperty::identifier());
            }
        }

        if ($builder instanceof LegacyItemBuilder && $blockBuilder !== null && $blockBuilder->getBlock() instanceof Crops) {
            if (!$builder->hasComponent(SeedComponent::identifier())) {
                $builder->addComponent(SeedComponent::fromBlocks($blockBuilder->getBlock(), VanillaBlocks::FARMLAND()));
            }
        }
    ...
    private function createDefaultBlockItemBuilder(BlockBuilder $builder): ItemBuilder {
        $block = $builder->getBlock();
        if ($block instanceof Crops) {
            $item = new #[LegacyItem] class(new ItemIdentifier(-$block->getTypeId()), $block->getName(), clone $block) extends Item implements AutoGeneratedBlockItem {
        ...
        $item = new #[DataDrivenItem] class(new ItemIdentifier(-$block->getTypeId()), $block->getName(), clone $block) extends Item implements AutoGeneratedBlockItem {
            public function __construct(
                ItemIdentifier $identifier,
                string $name,
                private readonly Block $block
        ...
        return DataDrivenItemBuilder::create($item)
            ->setRuntimeId($builder->getRuntimeId())
            ->setTypeId($item->getTypeId());
    }
